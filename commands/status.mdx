---
title: "/pm:status — View the Current Project Briefing"
sidebarTitle: "/pm:status"
description: "Re-render PROJECT.md from state.json and display the full conductor briefing: active epic, detour stack, next-up queue, lane counts, and tracker sync."
---

`/pm:status` re-renders `PROJECT.md` from the current state and summarizes the project's position: what's actively being built, what's paused and why, what's queued next, and how many epics exist per lane. It's the fastest way to re-orient yourself at the start of a session or after any state-changing operation.
