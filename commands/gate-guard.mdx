---
title: "/pm:gate-guard — Inspect the Reconcile Gate Guard"
...
As of the `gate-guard-default-on-reconcile` change, this check is **always active** for any epic carrying `reconcileNeeded: true` — including epics that already had the flag before this change was introduced. It previously required an explicit `set-gate-guard on`; real-usage feedback showed that opt-in was never turned on across several sessions where it would have caught a real skip, so the default was flipped.

**There is no bypass for this case.** Running `set-gate-guard off` does **not** silence the reconcile-owed block. The only path through it is completing the reconcile gate: run `/pm:resume`, delegate to the reconciler agent per the POP protocol, and let the reconcile gate clear `reconcileNeeded`. Once cleared, file writes proceed as normal.
