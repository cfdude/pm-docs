...
conductor.mjs triage "--story <n> is 1-indexed but --help says only '<a value>'"   # 0.41.0: an ask may begin with --
```
...
Quote the ask: it is ONE argument, and a second unquoted word is refused as an extra argument rather than silently dropped (0.44.0). It may stand before or after `--limit` — `triage --limit 3 "fix the render stamp"` reads the ask — while `triage --limit 5` with no ask still prints usage. A quoted ask that begins with `--` but is not shaped like a flag (`"--story <n> is 1-indexed"`) is the ask, not a flag.
