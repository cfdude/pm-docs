---
title: "/pm:init — Initialize PM in Your Project"
sidebarTitle: "/pm:init"
description: "Scaffold .conductor/state.json, register existing epics, write the CLAUDE.md rules block, and render PROJECT.md. Safe to re-run — it's a no-op if already initialized."
---

`/pm:init` bootstraps PM in your project. Run it once in any directory — it creates the `.conductor/` state directory, scans for existing OpenSpec proposals and Superpowers plans to register as epics, writes the managed rules block into `CLAUDE.md`, and renders the initial `PROJECT.md`. Until you run this command, all of PM's hooks stay dormant in your project, so this is always your first step.
...
<CardGroup cols={2}>
  ...
  <Card title=".conductor/detours.log" icon="scroll">
    An append-only trail of every PUSH and POP event in the detour stack. Empty on first init — entries accumulate as you use `/pm:detour` and `/pm:resume`.
  </Card>
...
<Steps>
  ...
  <Step title="Scans for existing work">
    PM scans for OpenSpec change directories (`openspec/changes/<id>/`) and Superpowers plan files. Any found are registered as `untriaged` epics in the appropriate lane so no prior work is lost.
  </Step>
...
NEXT UP
  ...
  [P?] dashboard-redesign   untriaged   superpowers

LANE COUNTS
  openspec     1 epic   (1 untriaged)
  superpowers  1 epic   (1 untriaged)
...
<Note>
  PM's hooks — `SessionStart`, `PreCompact`, `PostToolUse`, and `PreToolUse` — stay dormant in your project until `/pm:init` runs. This is intentional, matching the same design as `openspec init`. Running init is what arms the hooks for all future sessions.
</Note>
