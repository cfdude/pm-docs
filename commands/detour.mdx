...
When PM classifies a detour as substantial, it performs the following steps automatically:

1. Makes the current epic's `tasks.md` reflect reality and commits so nothing is left uncommitted.
2. Sets the current epic's `status` to `paused` in `.conductor/state.json`.
3. Pushes a stack frame with `pausedEpic`, `pausedAt`, `reason`, `spawnedDetour`, and `reconcileOnResume: true` (default true whenever the detour will touch code the paused epic depends on).
4. Creates the detour as a new epic with `role: detour` at P0 by default, in the appropriate lane.
5. Records cross-links: the detour gets a `resolves-blocker-for` link pointing to the parent; the parent gets a `may-invalidate` link pointing to the detour.
6. Sets the detour epic as active and opens its OpenSpec proposal.
