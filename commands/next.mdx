---
title: "/pm:next — Get a Concrete Work Recommendation"
sidebarTitle: "/pm:next"
...
Once PM identifies the next epic, it makes that epic active through the engine CLI:

```bash
node "$ENGINE" set-active <epic-id>
```

`set-active` does three things atomically: it updates the top-level `.active` pointer in `state.json`, sets the chosen epic's `status` to `active`, and demotes any previously-active epic back to `queued`. This keeps the briefing's **NOW** line accurate without requiring you to manually update state.
