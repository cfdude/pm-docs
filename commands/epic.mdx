...
| `--deferral` | `deferralAssertion` | **Repeatable** — `"<epicId>:<section>"`. Splits on the first colon, which is correct here because the left half is an epic id and cannot contain one; a colon in `<section>` is part of the section. |
