...
<Steps>
  ...
  <Step title="Start building">
    ...
    - After each `git commit`, a hook on `PostToolUse` **and** `PostToolUseFailure` nudges you to update the epic's status with `update-epic`, reports every commit that landed since its last observation, and auto-detects whether the commit shape looks like an unlogged minimal detour.
