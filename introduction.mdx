...
| Metric | Value |
| --- | --- |
| Releases shipped end-to-end (spec → build → test → changelog → release) | **54** |
| Tests in the engine | **1,069** |
| Lines of engine code (`scripts/conductor.mjs` \+ `scripts/lib/*.mjs`) | **~12,300** |
...
**What your engine accepts: ask your engine.** Since 0.37.0, `--help` is per-verb — `node "$ENGINE" <verb> --help` prints that verb's real flag surface: every flag, which take no value, which repeat, and the legal values of every closed enum (`--outcome`, `--status`, `--lane`, `--priority`, `--platform`). It is projected from the same registry rows the engine's unknown-flag guards read, so it is version-exact by construction and cannot advertise a flag your parser refuses. Reach for it before reading engine source, and before guessing a flag name.
