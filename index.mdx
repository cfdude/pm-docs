...
**Project Manager (PM)** is a lightweight harness above [OpenSpec](https://github.com/Fission-AI/OpenSpec)
and [Superpowers](https://github.com/obra/superpowers) that keeps a Claude Code project on
track — across detours, context compaction, and however many epics are in flight at once.

Install it once, run `/pm:init`, and it stays oriented across every context compaction. Zero
npm dependencies. Pure Node 18+ built-ins.
...
<CardGroup cols={3}>
  ...
  <Card title="Zero Dependencies" icon="feather">
    The engine is pure Node 18+ built-ins. Nothing to `npm install`, ever. See the [Introduction](/introduction) for current test/line counts.
  </Card>
