...
- **How to turn a preflight scan into a real safety mechanism** — epic-level autonomy's decision rule (pre-authorized → proceed; no backup path → hard stop; destructive-but- restorable → warn and log; genuine unknown → stop) is designed to be followed by an agent mid-task, not just read once at the start. **A grant taken back with `set-autonomy --revoke` covers nothing, and neither does one naming nothing**: the engine keeps a revoked grant as record rather than as authorisation, and the agent is the only actor that acts on the grant list, so a grant the engine considers dead must not be honoured in the decision (0.46.0).
