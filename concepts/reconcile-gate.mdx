...
The gate guard is a `PreToolUse` hook that mechanically blocks `Edit`, `Write`, and `NotebookEdit` — and, on a `Bash` call, a member of a closed, documented list of write shapes (0.46.0) — while the active epic has `reconcileNeeded: true`. It is:
...
- **Closed when state cannot be read** (0.44.0) — while `.conductor/state.json` exists but cannot be read, the hook blocks `Edit`/`Write`/`NotebookEdit` and every Bash write shape too, because whether a reconcile is owed is then unknown — with a carve-out that an affirmed `Bash` call carrying a command is allowed whatever its shape, so the git remedies stay runnable; see `/pm:gate-guard`.
