...
├── commit-watch.json    # HEAD watermark the commit hook observes (git-ignored)
...
**Never hand-repair it by guessing which side of a conflict to keep** — that is the user's call. Meanwhile the hooks never write over it: `gate-guard` blocks `Edit`/`Write`/`NotebookEdit` (exit 2) and Bash is not matched, so the remedies run from the shell; `brief` injects only the warning; `snapshot` writes nothing (exit 11, never 2, which would block compaction); `commit-nudge` writes nothing but its HEAD watermark (exit 2 when a commit has landed; 0 otherwise, without reading state). `verify-state` never loads the file, and `activity` reports the revision and the log's on/off state as unknown.
