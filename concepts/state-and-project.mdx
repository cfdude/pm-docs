...
**Never hand-repair it by guessing which side of a conflict to keep** — that is the user's call. Meanwhile the hooks never write over it: `gate-guard` blocks `Edit`/`Write`/`NotebookEdit` and, on `Bash`, only a member of its closed list of write shapes (exit 2) — but with a carve-out, since 0.46.0, that an affirmed `Bash` call carrying a command is allowed even over an unreadable record, so the remedies still run from the shell; `brief` injects only the warning; `snapshot` writes nothing (exit 11, never 2, which would block compaction); `commit-nudge`, on `PostToolUse` and `PostToolUseFailure` alike, writes nothing — the observation record included, so a commit that landed meanwhile is reported by the first run after the file is repaired (exit 2 when a commit has landed; 0 otherwise, without reading state). `verify-state` never loads the file, and `activity` reports the revision and the log's on/off state as unknown.
...
| `detourStack[]` | `array` | Stack frames, LIFO order. Each frame has `pausedEpic`, `pausedAt`, `reason`, `spawnedDetour`, and `reconcileOnResume`. As of 0.44.0 the paused epic's `may-invalidate` link carries its own `reconcileOnResume` too — the ARMING record, `true` only from `push-detour --reconcile` — plus `reconciled: {verdict, amendments, reconciledAt}` once answered, a drop stamp (`droppedAt` + the reason) when the obligation was ended with `drop-detour` rather than answered (0.46.0), and `superseded` holding the previous verdict after a correction or re-arm. |
