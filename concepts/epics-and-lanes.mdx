...
| `may-invalidate` | The linked epic's design **may have been invalidated** by this epic's changes. Set on the parent when a detour is pushed. This is the reconcile relationship — it connects a detour to the epic it might have made stale. Carries the arming record `reconcileOnResume` (0.44.0): `true` only from `push-detour --reconcile`, `false` from `--no-reconcile` or a link supplied by hand, and `record-reconcile` answers only an armed link. |
