...
<Steps>
  ...
  <Step title="Register the detour epic">
    Pausing the current epic is never a hand-edit: `push-detour` below writes `status: paused` itself, in the same guarded write as the frame. What this step needs is the detour registered as its own epic — the appropriate lane, usually priority `P0` — so the PUSH has something to name.
  </Step>
...
<Steps>
  ...
  <Step title="Restore the paused epic to active">
    `pop-detour` already did this — it resumes the paused epic and moves the active pointer in the same write that removes the frame. Nothing here sets a `status` by hand; `set-active <paused-id>` is only needed if the pointer was moved elsewhere in between.
  </Step>
