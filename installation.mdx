...
<Info>
  **Node 22\+** is required to run the conductor engine — pm supports the oldest Node LTS line that is not end-of-life, which is 22 today. If the Node running pm is below that, the `SessionStart` briefing warns in one line (a warning, never a refusal). **Claude Code** is the only supported platform today — Codex, Gemini CLI, and other platforms are on the roadmap.
</Info>
