...
3. **Run the reconcile gate** — if the stack frame had `reconcileOnResume: true`, PM delegates a fresh-context review to the `reconciler` agent via the Task tool, passing the paused epic id and the detour epic id. The reconciler re-reads the paused proposal cold, diffs what the detour actually shipped, and reports back a verdict.
...
The `reconciler` agent runs in a completely fresh context. It has no memory of the original build session. Given the paused epic id and the detour epic id, it:
...
**`invalidated`** — the detour changed something the paused proposal depends on. The reconciler lists specific amendments: stories to add, remove, or rewrite. Update the proposal and `tasks.md` before writing any code.
