...
<Steps>
  ...
  <Step title="Cross-links are recorded">
    Two links are written automatically: the detour `resolves-blocker-for` the paused parent, and the parent `may-invalidate` the detour. These links are what the reconcile gate reads when you resume. As of 0.44.0 the parent's `may-invalidate` link records the reconcile choice per detour (`reconcileOnResume`), a `--no-reconcile` push never cancels an earlier obligation, and while a verdict is owed `update-epic <parent> --clear-links` and `remove-epic <detour>` are refused — see `/pm:detour` and `/pm:resume`.
  </Step>
...
4. **Write the verdict back durably** — this is what actually clears `reconcileNeeded`. The result is not just narrated in the transcript; it is attached to the paused epic's link in `.conductor/state.json` via `record-reconcile`. As of 0.44.0 there is one form — `--amendments none`, or one `--amendment "<line>"` per AMENDMENTS line — the verdict is accepted only against a detour the epic was paused for with `--reconcile` after its frame is popped, and `reconcileNeeded` clears only when no armed detour is left unanswered.
